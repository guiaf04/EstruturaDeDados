...
static 
NoEnc *lista_enc_obter_no_indice(ListaEncInt *lista, unsigned int indice) {
...
void lista_enc_remover_entre_it(ListaEncInt *lista, ItListaEncInt *inicio, ItListaEncInt *fim) {
...
void lista_remendar(ListaEncInt *l1, ListaEncInt *l2, int k){
    NoEnc *atual_l1 = l1->primeiro;
    NoEnc *atual_l2 = l2->primeiro;
    ...
    atual_l1 = l1->primeiro;

    if(k <= 0 && l1->primeiro != NULL && l2->primeiro != NULL){
      NoEnc *auxiliar = l1->primeiro;
      l1->primeiro = l2->primeiro;
      NoEnc *atual = l1->primeiro;
    ...
    if(k > tamanho && l1->primeiro != NULL && l2->primeiro != NULL){
      NoEnc *atual = l1->primeiro;

      while (atual->proximo != NULL){
        atual = atual->proximo;
      }
      atual->proximo = l2->primeiro;
      l2->primeiro = NULL;
    }else
    if(l1->primeiro != NULL && l2->primeiro != NULL){
      while(atual_l1->proximo != NULL){
          int cont = 0;
          NoEnc *auxiliar = atual_l1->proximo;

          if(cont = k - 1){
            atual_l1->proximo = l2->primeiro;
            
            while(atual_l2->proximo != NULL){
              atual_l2 = atual_l2->proximo;
            }
            atual_l2->proximo = auxiliar;
          }
